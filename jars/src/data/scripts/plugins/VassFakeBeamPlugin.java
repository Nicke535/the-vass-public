...
public class VassFakeBeamPlugin extends BaseEveryFrameCombatPlugin {
    ...
    private SpriteAPI beam = Global.getSettings().getSprite("beams", "vass_fakeBeamFX");
        ...
        if (!FAKEBEAMS.isEmpty()){
            ...
            for (Map< String,Float > entry : FAKEBEAMS) {
                ...
                if (time <= 0){
                    ...
                    render(
                            beam, //Sprite to draw
                            entry.get("w") * opacity, //Width entry shrinking with the opacity
    ...
    private void render ( SpriteAPI sprite, float width, float height, float angle, float opacity, float posX, float posY){
        //Since only the Makhaira use fakebeams (and only with the Isochronal Multilinker) create a random color every frame, to match the Multa aesthetic
        Color color = new Color(230 + MathUtils.getRandomNumberInRange(0, 25), 230 + MathUtils.getRandomNumberInRange(0, 25), 230 + MathUtils.getRandomNumberInRange(0, 25));
