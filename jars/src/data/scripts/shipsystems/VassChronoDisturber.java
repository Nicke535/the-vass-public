...
public class VassChronoDisturber extends BaseShipSystemScript {
	...
	public static final float ACTIVE_RANGE = 900f;
    ...
    public static final float AFTERIMAGE_DELAY_MAX = 0.3f;
        ...
        for (ShipAPI shp : Global.getCombatEngine().getShips()) {
            ...
            if (MathUtils.getDistance(shp, ship) <= (ACTIVE_RANGE)) {
                ...
                if (shp.getOwner() != ship.getOwner()) {
					...
					//And some afterimages, to further give you a hint that you are being time-disrupted (ignore fighters though: the performance hit isn't worth it)
                    if (shp.getHullSize() == ShipAPI.HullSize.FIGHTER) {
                        //We don't need to worry about the player being a fighter, so we can just continue
                        continue;
                    }
                    if (Global.getCombatEngine().getCustomData().get("VassChronoDisturberAfterimageTracker" + shp.getId()) instanceof Float) {
                        Global.getCombatEngine().getCustomData().put("VassChronoDisturberAfterimageTracker" + shp.getId(), (float)Global.getCombatEngine().getCustomData().get("VassChronoDisturberAfterimageTracker" + shp.getId()) + Global.getCombatEngine().getElapsedInLastFrame());

                        if ((float)Global.getCombatEngine().getCustomData().get("VassChronoDisturberAfterimageTracker" + shp.getId()) > (AFTERIMAGE_DELAY_MAX * shp.getMutableStats().getDynamic().getStat("VassChronoDisturberTracker" + id).getModifiedValue())) {
                            Vector2f targetVector = VectorUtils.getDirectionalVector(shp.getLocation(), ship.getLocation());
                            targetVector.scale(650f);
                            shp.addAfterimage(JITTER_COLOR, 0f, 0f, targetVector.x, targetVector.y, 1f,0f, 0f, 0.3f, true, false,false);
                            Global.getCombatEngine().getCustomData().put("VassChronoDisturberAfterimageTracker" + shp.getId(),0f);
                        }
                    } else {
                        Global.getCombatEngine().getCustomData().put("VassChronoDisturberAfterimageTracker" + shp.getId(),0f);
                    }
					...
					if (shp == Global.getCombatEngine().getPlayerShip()) {
						...
						Global.getCombatEngine().maintainStatusForPlayerShip("VassChronoDisturberDebugInfoID","graphics/vass/icons/hullsys/chrono_disturber.png", "Chrono Disturber", "Timeline partially disrupted", true);
					}
                }
            }
