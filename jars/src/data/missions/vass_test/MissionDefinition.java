...
public class MissionDefinition implements MissionDefinitionPlugin {
	...
	public void defineMission(MissionDefinitionAPI api) {
		...
		api.initFleet(FleetSide.PLAYER, "VSS", FleetGoal.ATTACK, false);
...
//		api.getDefaultCommander(FleetSide.PLAYER).getStats().setSkillLevel(Skills.ELECTRONIC_WARFARE, 3);
		...
		api.setFleetTagline(FleetSide.PLAYER, "Feeding the addiction");
		...
		for (String testID : Global.getSettings().getAllVariantIds()) {
			if (testID.contains("vass_") && Global.getSettings().getVariant(testID).getSource() == VariantSource.HULL && !Global.getSettings().getVariant(testID).getHullSize().equals(ShipAPI.HullSize.FIGHTER)) {
