...
public class VassPeriodicPlating extends BaseHullMod {
    ...
    //public static final Color AFTERIMAGE_COLOR = new Color(255, 255, 255, 100);
	...
	public void advanceInCombat(ShipAPI ship, float amount) {
		if (!ship.getSystem().isActive() && !ship.getFluxTracker().isVenting() && !ship.isHulk()) {
			if (ship == Global.getCombatEngine().getPlayerShip()) {
				float shipTimeMult = 1f + (TIME_MULT - 1f);
				ship.getMutableStats().getTimeMult().modifyMult("VassPeriodicPlatingDebugID", shipTimeMult);
				Global.getCombatEngine().getTimeMult().modifyMult("VassPeriodicPlatingDebugID", 1f / shipTimeMult);
			} else {
				float shipTimeMult = 1f + (TIME_MULT - 1f);
				ship.getMutableStats().getTimeMult().modifyMult("VassPeriodicPlatingDebugID", shipTimeMult);
