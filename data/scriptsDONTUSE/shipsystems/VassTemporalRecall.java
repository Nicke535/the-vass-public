        ...
        if (effectLevel > 0.1f && hasATarget) {
            ...
            if (state == State.ACTIVE && hasATarget) {
                ...
                while (particleCount > 0) {
                    Global.getCombatEngine().addSmoothParticle(MathUtils.getRandomPointInCircle(ship.getLocation(), 10), MathUtils.getRandomPointInCircle(new Vector2f(0f, 0f), particleSpread), (float)Math.random() * 40f, (float)Math.random(), (float)Math.random() * 2f, JITTER_UNDER_COLOR);
                    particleCount--;
                }
